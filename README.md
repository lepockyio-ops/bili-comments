...
> B 站视频评论收集器 · 输入视频链接 → 输出 Google Sheets 友好的 XLSX（含中→日自动翻译公式）。为日本 Vocaloid / 歌い手 / 曲师看粉丝反馈场景设计。
...
- 🌐 **一键抓取** — 输入 bvid 或链接，输出 XLSX
- 🇯🇵 **中→日自动翻译** — 内置 `=GOOGLETRANSLATE` 公式，上传 Google Sheets 后自动执行
- 🎯 **智能筛选辅助** — 3 个 Sheet：全部评论 / 视频信息 / Top 20 高赞
- 🎨 **UP 主评论高亮** — 曲师/UP 本人的评论黄色背景，一眼识别
